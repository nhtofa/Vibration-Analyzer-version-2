...
(function () {
  ...
  function drawLines(canvas, datasets, options = {}) {
    ...
    ctx.clearRect(0, 0, width, height); ctx.fillStyle = '#ffffff'; ctx.fillRect(0, 0, width, height); ctx.strokeStyle = '#e3e9f2'; ctx.fillStyle = '#5c6b7d'; ctx.font = '11px system-ui';
    ...
    ctx.beginPath(); ctx.strokeStyle = '#b9c4d4'; ctx.moveTo(pad.left, pad.top); ctx.lineTo(pad.left, height - pad.bottom); ctx.lineTo(width - pad.right, height - pad.bottom); ctx.stroke();
    datasets.forEach((dataset, datasetIndex) => { ctx.beginPath(); ctx.strokeStyle = dataset.color || (datasetIndex ? '#16a34a' : '#0d6efd'); ctx.lineWidth = 1.6; dataset.y.forEach((value, index) => { const px = pad.left + ((dataset.x[index] - minX) / (maxX - minX)) * plotWidth; const py = pad.top + ((maxY - value) / (maxY - minY)) * plotHeight; if (index === 0) ctx.moveTo(px, py); else ctx.lineTo(px, py); }); ctx.stroke(); });
    ctx.fillStyle = '#5c6b7d'; ctx.fillText(options.xLabel || 'x', width / 2 - 10, height - 8); ctx.save(); ctx.translate(13, height / 2 + 22); ctx.rotate(-Math.PI / 2); ctx.fillText(options.yLabel || 'amplitude', 0, 0); ctx.restore();
  }
  function drawHeatmap(canvas, result) {
    const { ctx, width, height } = canvasSize(canvas); const pad = { left: 55, right: 15, top: 15, bottom: 34 }; const plotWidth = width - pad.left - pad.right; const plotHeight = height - pad.top - pad.bottom; const rows = result.matrix.length; const cols = result.times.length; ctx.clearRect(0, 0, width, height); ctx.fillStyle = '#ffffff'; ctx.fillRect(0, 0, width, height);
    let peak = 0; result.matrix.forEach((row) => row.forEach((value) => { if (value > peak) peak = value; })); peak = peak || 1;
    for (let y = 0; y < rows; y += 1) for (let x = 0; x < cols; x += 1) { const intensity = Math.sqrt((result.matrix[y][x] || 0) / peak); const r = Math.floor(232 - 219 * intensity); const g = Math.floor(241 - 155 * intensity); const b = Math.floor(251 - 50 * intensity); ctx.fillStyle = `rgb(${r},${g},${b})`; ctx.fillRect(pad.left + x * plotWidth / cols, pad.top + (rows - y - 1) * plotHeight / rows, Math.ceil(plotWidth / cols) + 1, Math.ceil(plotHeight / rows) + 1); }
    ctx.strokeStyle = '#b9c4d4'; ctx.strokeRect(pad.left, pad.top, plotWidth, plotHeight); ctx.fillStyle = '#5c6b7d'; ctx.font = '11px system-ui'; ctx.fillText('time (s)', width / 2 - 15, height - 8); ctx.save(); ctx.translate(13, height / 2 + 22); ctx.rotate(-Math.PI / 2); ctx.fillText('frequency (Hz)', 0, 0); ctx.restore();
  }
  ...
  function renderEnvelope(series) {
    ...
    drawLines(card('Band-pass filtered signal', true), [{ x: series.time, y: result.filtered }], { xLabel: 'time', yLabel: 'amplitude' }); drawLines(card('Hilbert envelope', true), [{ x: series.time, y: result.envelope, color: '#16a34a' }], { xLabel: 'time', yLabel: 'envelope' });
    const visible = result.spectrum.frequencies.map((frequency, i) => i).filter((i) => result.spectrum.frequencies[i] <= max); drawLines(card('Envelope spectrum', true), [{ x: visible.map((i) => result.spectrum.frequencies[i]), y: visible.map((i) => result.spectrum.magnitude[i]), color: '#6f42c1' }], { xLabel: 'frequency (Hz)', yLabel: 'amplitude' });
  }
  ...
  function renderSpectrogram(series) {
    ...
    drawHeatmap(card('Spectrogram (time · frequency · magnitude)', true), result); drawLines(card('Dominant frequency over time'), [{ x: result.times, y: result.dominant.map((item) => item.frequency), color: '#16a34a' }], { xLabel: 'time (s)', yLabel: 'frequency (Hz)' }); drawLines(card('Vibration energy over time'), [{ x: result.times, y: result.dominant.map((item) => item.magnitude ** 2), color: '#6f42c1' }], { xLabel: 'time (s)', yLabel: 'energy' });
  }
  function render() {
